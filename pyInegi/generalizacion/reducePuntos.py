...
def crearMapa(**capas):
	...
	geomDF=capas['c']
	imp("Creando Mapa...")
	m1=_cap["Visible"][0].explore(color="gray",tooltip=True,marker_type="circle",style_kwds=dict(color=_cap['Visible'][1],weight=5,opacity=1),marker_kwds=dict(radius=4, fill=True,draggable=True),legend=False,name="Visibles",popup=True)
		...
		if _cantG>1:
			...
			for _g in gpoX.index:
				...
				if res.loc[_g,"isvisible"]:
					if data:=[dict(idx=i,jer=j) for i,v,j in zip(res.index,res['isvisible'],res[nom_camp[0]]) if v and i!=_g]:
